...
check_tls13_openssl() {
  ...
  until output=$(echo | openssl s_client -connect localhost:$local_port -tls1_3 2>/dev/null); do
    ...
    echo "[$name] Waiting for port-forward (openssl)... attempt $attempt/$max_retries"
    sleep $retry_delay
    ((attempt++))
  done
