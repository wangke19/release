...
if [[ -z "${HOSTED_CP:-}" ]]; then
  ...
  HYPERSHIFT_MANAGEMENT_CLUSTER_NAMESPACE="$(oc get hcp -A -o=jsonpath="{.items[?(@.metadata.name==\"$CLUSTER_NAME\")].metadata.namespace}")"
  export HYPERSHIFT_MANAGEMENT_CLUSTER_NAMESPACE
  HYPERSHIFT_NAMESPACE="$(oc get hostedclusters -A -o=jsonpath="{.items[?(@.metadata.name==\"$CLUSTER_NAME\")].metadata.namespace}")"
  export HYPERSHIFT_NAMESPACE
...
sleep 5h
echo "openshift-tests run ${TEST_SUITE} ${TEST_ARGS:-} --provider \"${TEST_PROVIDER}\" -o \"${ARTIFACT_DIR}/e2e.log\" --junit-dir \"${ARTIFACT_DIR}/junit\" > /tmp/run"
